...
// If 'rep_out' isn't NULL and no equivalent was found, '*rep_out' is set to
// NULL.
// !! If 'rep_out' is NULL and there is an equivalent, the memory is leaked!!
// So make sure to have 'out' not NULL or there is no equivalent.
...
int tlht_remove(tl_ht *, tlht_bucket *search_bucket, tlht_cmp_func *cmp,
                tlht_bucket **out);
// Searches for an equivalent bucket, and sets *out to the finding or NULL.
// Calling it with out == NULL is meaningless.
...
// to never expand.
//
// Sets '*old_buckets' to old buckets if resized, otherwise to NULL
// 'old_buckets' pointer can't be NULL.
// If the return value isn't 0, ''*old_buckets' isn't touched
//
//
